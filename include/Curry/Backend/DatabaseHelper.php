...
class Curry_Backend_DatabaseHelper {
	 ...
	 * @param \Curry\Backend|null $backend
	 * @return bool	True on success
	 */
	public static function dumpDatabase($file, $tables = null, \Curry\Backend $backend = null)
	{
	 ...
	 */
	public static function dumpTable($table, $fp, &$error, \Curry\Backend $backend = null)
	{
		$goodRows = 0;
		$totalRows = 0;
		...
		try {
			...
			foreach($query->find() as $obj) {
				...
				try {
					...
					if($backend)
						$backend->addMessage('Unable to dump row: '.$e->getMessage(), \Curry\Backend::MSG_ERROR);
					$error = true;
				}
			}
			if($backend)
				$backend->addMessage("Dumped $goodRows / $totalRows rows in table $table", $goodRows == $totalRows ? \Curry\Backend::MSG_SUCCESS : \Curry\Backend::MSG_ERROR);
		}
		catch (Exception $e) {
			if($backend)
				$backend->addMessage('Unable to dump table: '.$e->getMessage(), \Curry\Backend::MSG_ERROR);
			$error = true;
		}
		return $goodRows;
	}
		...
		foreach($query->find() as $obj) {
				...
				if($column->isForeignKey()) {
					...
					if(!$relObjects) {
						if($column->isNotNull()) {
							if($delete) {
								if($backend)
									$backend->addMessage("Deleting $objName (required $columnName was invalid).", \Curry\Backend::MSG_WARNING);
								$obj->delete();
								$error = array();
							...
							if($fix) {
								// attempt to fix
								if($backend)
									$backend->addMessage("Fixing $objName (invalid $columnName will be set to null).", \Curry\Backend::MSG_WARNING);
			...
			if(count($error)) {
				...
				if($backend) {
					...
					$backend->addMessage("$link: ".join(', ', $error).'.', \Curry\Backend::MSG_WARNING, false);
				}
			}
		}
	 ...
	 */
	public static function restoreFromFile($file, $tables = null, $maxExecutionTime = 0, $continueLine = 0, \Curry\Backend $backend = null)
	{
		...
		if(is_array($header) && isset($header['header'])) {
			...
			if ($pageVersion !== Page::VERSION) {
				if ($backend)
					$backend->addMessage("Migrating data from version $pageVersion to ".Page::VERSION, \Curry\Backend::MSG_WARNING);
				...
				foreach($classes as $table) {
					try {
						...
						if(!method_exists($table, 'delete')) {
							if($backend)
								$backend->addMessage("Skipping read-only table: $table", \Curry\Backend::MSG_WARNING);
							continue;
						}
		...
		while (!feof($fp)) {
			...
			if(is_array($data) && isset($data['table'])) {
				...
				if($data['table'] !== $currentTable && $currentTable !== null && $backend) {
					...
					if(count($added))
						$backend->addMessage('New column(s): '.join(', ', $added), \Curry\Backend::MSG_WARNING);
					if(count($removed))
						$backend->addMessage('Removed column(s): '.join(', ', $removed), \Curry\Backend::MSG_WARNING);
				}
				...
				if($backend)
					$backend->addMessage('Unable to read data on line '.$total, \Curry\Backend::MSG_ERROR);
				++$failed;
			}
		...
		if($backend) {
			...
			if($failed)
				$backend->addMessage("Failed to add $failed rows", \Curry\Backend::MSG_ERROR);
			$backend->addMessage("Added " . ($total - $skipped - $failed) . " / $total rows in ".round(microtime(true) - $t, 2)."s", !$failed ? \Curry\Backend::MSG_SUCCESS : \Curry\Backend::MSG_ERROR);
		}
