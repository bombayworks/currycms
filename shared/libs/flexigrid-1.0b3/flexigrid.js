...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		var g = {
							 	...
							 	function ()
									{
										...
										// ADDED BY JESPER EK
										if(p.colModel[idx].name)
											$.data(tr, p.colModel[idx].name, row.cell[idx]);
										/////////////////////
									}
							); 
			...
			populate: function (opts) { //get latest data
				...
				if(opts && opts.params) {
					$.each(opts.params, function(k, v) {
						if(typeof(v) == 'object') {
							$.each(v, function() {
								param[param.length] = {name: k, value: this};
							});
						} else {
							param[param.length] = {name: k, value: v};
						}
					});
				}
				///////////////////////
		...
		if (p.searchitems)
			{
				...
				// ADDED BY JESPER EK
				// webkit or jquery bug
				//$('input[name=q],select[name=qtype]',g.sDiv).keydown(function(e){if(e.keyCode==13) g.doSearch()});
				$('input[name=q]',g.sDiv).keydown(function(e){if(e.keyCode==13) g.doSearch()});
	...
	$.fn.flexReload = function(opts) { // function to reload grid

		return this.each( function() {
				if (this.grid&&this.p.url) this.grid.populate(opts);
			});
